...
parser.add_argument('--img_size', type=int, default=162, help='size of each image dimension')
...
# parser.add_argument('--train', type=str, default='True', help='train the network', action='store_true')
# parser.add_argument('--impute', type=str, default='True', help='do imputation', action='store_true')
...
# parser.add_argument('--file_d', type=str, default='scIGANs-scIGANs_0.1.1/ercc.txt', help='path of data file')
# parser.add_argument('--file_c', type=str, default='scIGANs-scIGANs_0.1.1/test_data/ercc.label.txt', help='path of cls file')
...
#%% for debug use only
opt.file_d='D:/bigdata/新加实验5.20/processed_data/petropoulos.csv'
opt.file_c='D:/bigdata/新加实验5.20/processed_data/petropoulos.txt.label.csv'
opt.img_size=162
opt.train = True
opt.impute = True
opt.n_epochs = 200
...
if opt.train:
    ...
    print("Training was started at", starttime)
    """0801, 加载之前训练好的预训练权重"""
#    if opt.dpt!='' and cuda==True:
    if cuda==True:
        discriminator.load_state_dict(torch.load("D:/bigdata/scIGANsaaa/--100-200-14-d.pt"))
        generator.load_state_dict(torch.load("D:/bigdata/scIGANsaaa/--100-200-14-g.pt"))
        print("use GPU")
#    if opt.dpt!='' and cuda != True:
    if cuda!=True:
        #discriminator.load_state_dict(torch.load("D:/bigdata/scIGANsaaa/--100-200-14-d.pt", map_location=lambda storage, loc: storage))
        weights_dict_d = torch.load("D:/bigdata/scIGANsaaa/--100-200-14-d.pt", map_location=lambda storage, loc: storage)
        load_weights_dict_d = {k: v for k, v in weights_dict_d.items()
                             if k not in['pre.0.weight','conv_blocks02p.1.weight'] }
        discriminator.load_state_dict(load_weights_dict_d, strict=False)
        #generator.load_state_dict(torch.load("D:/bigdata/scIGANsaaa/--100-200-14-g.pt", map_location=lambda storage, loc: storage))
        weights_dict_g = torch.load("D:/bigdata/scIGANsaaa/--100-200-14-g.pt", map_location=lambda storage, loc: storage)
        load_weights_dict_g = {k: v for k, v in weights_dict_g.items()
                             if k not in ['l1.0.weight','l1.0.bias','l1p.0.weight','l1p.0.bias','conv_blocks_02p.1.weight']}
        generator.load_state_dict(load_weights_dict_g, strict=False)
        print("use CPU")
    ...
    for epoch in range(opt.n_epochs):
        ...
        if cur_M < max_M:  # if current model is better than previous one
            ...
            if dM < min_dM:  # if convergence threthold meets, stop training
                ...
                print("Training was stopped at", endtime)
...
"""        # 指定epoch
        if epoch == opt.n_epochs:
            ...
            print(
                "Training was stopped after " + str(epoch) + " epoches since the convergence threthold (" + str(
                        min_dM) + ".) reached: " + str(dM))
            break
"""
...
if opt.impute:
    ...
    pd.DataFrame(rels).to_csv(
        os.path.dirname(os.path.abspath(opt.file_d)) + '/scGCT-100' + job_name + '.csv')  # imped data
